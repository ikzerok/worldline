...
冲突方案通过 `apply_proposal_with_resolutions` 明确提交；每项按 core 预览返回的 `{path, location}` 唯一寻址，须覆盖全部冲突，重复、缺失或额外方案一律拒绝。非空 `location` 是 JSON Pointer，`value` 必须是完整 JSON 值文本；数组并发修改作为数组路径上的一个整体冲突，按完整数组替换，不生成数组索引级冲突。`None` 只删除对象成员；结构化 JSON 根冲突必须提供完整 JSON 文本。空位置的文件冲突接受完整 UTF-8 文件文本或 `None`（删除文件）。core 重新检查提案、修订、内容基线和当前冲突集合；JSON 无法解析、内容候选编译失败或仍有冲突时，整个候选零写入。方案应用不改写提案内的 base/proposed，也不改变当前稿原件；仅在所有冲突经 core 校验后，才在同一候选中提交解决结果并把提案标记为 `accepted`。
...
| 浏览器检查点快照 | 3 MiB 序列化 core 内容；浏览器存储配额可能更低 | 本地保存 envelope | CAP-04D |
...
| 检查点文本差异 | 256项/源码文件 | 恢复预览 | CAP-04D |
| 检查点文本片段 | 16 KiB/每侧/源码文件 | base/current/checkpoint 原文回退 | CAP-04D |
| 检查点文本投影文件 | 32个/恢复计划 | 恢复预览 | CAP-04D |
| 检查点文本差异片段 | 16 KiB/源码文件 | 差异项合计 | CAP-04D |
| 提案审阅受影响对象 | 256个/内容文件 | 引用影响 DTO | CAP-04A |
| 提案审阅入站引用 | 256条/对象/当前或候选 | 引用影响 DTO | CAP-04A |
| Markdown 导入文件数 | 512 个（Markdown 与附件合计） | 单次迁移 | CAP-07A |
| Markdown 文本大小 | 1 MiB/页；16 MiB/次合计 | front matter/正文 | CAP-07A |
| Markdown 附件大小 | 64 MiB/次合计 | 复制到工作区的本地附件 | CAP-07A |
| Markdown 引用数 | 4096 条/次 | 页面链接与附件引用合计 | CAP-07A |
| Markdown 扫描项数 | 2048 项 | 来源树中的目录与文件总数 | CAP-07A |
