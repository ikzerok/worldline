...
| P003 | 字符串未闭合或非法转义 |
| P004 | 期望 token 缺失或属性 `ref` 的目标 kind 不支持(如 `choice` 后缺标签) |
| P005 | 块为空或声明后无缩进体 |
| P006 | 表达式语法错误 |
| P007 | include 路径语法错误 |
...
| A214 | error | 标签标记、附件或显式对象属性引用了不存在的对象、标签或素材 |
...
| code | severity | 场景 |
|---|---|---|
| PRESET001 | error | 预设 JSON/结构、地图/网络/图层/范围引用或路径/分布区几何用途无效 |

PRESET 诊断只描述展示选择，不生成世界语义；原始预设字节仍由 Project 生命周期保留。

### COLLAB00x — 批注与提案

| code | severity | 场景 |
|---|---|---|
| COLLAB001 | error | 批注或提案 JSON、schema、注册 ID、字段结构或文档生命周期无效 |

批注失锚不是自动修复错误，而是显式 `detached` 状态；作者必须重新指定锚点。提案预览中的同字段、删改、数组并发修改是结构化冲突，不转换成诊断后静默覆盖。协作文档不进入故事 Program 或运行指纹。

### TPL00x — 工程模板

| code | severity | 场景 |
|---|---|---|
| TPL001 | error | 模板 JSON、重复键或顶层结构无效 |
| TPL002 | error | 模板 schema 版本或必需能力不受支持；原文只读保留 |
| TPL003 | error | 模板 ID 与注册键不一致，或模板 ID/路径冲突 |
